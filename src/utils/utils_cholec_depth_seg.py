...
class check_points():
    def __init__(self, folder="../src/result/model/train_valid_test"):
        """
...
class data_json():
    ...
    def train_valid_test(self, train_radio=0.6, \
        ...
        the form of cholec80_path: '../../data/styleFromCholec80/*/style../*.png'
        ...
        cholec80_path = [i.replace("simulated", "styleFromCholec80") \
                           .replace("inputs", "style_02") \
                       for i in train_path]
        depth_path = [i.replace("simulated", "simulated_depths") \
                           .replace("inputs", "depths") \
                           .replace(".png", '.exr').replace('img', 'depth')
                       for i in train_path]
        seg_path = [i.replace("simulated", "simulated_segmentation") \
        ...
        ret_Dic = {}
        train_Dataset = {'input': train_path[: train_Num], 'cholec80': cholec80_path[: train_Num],\
                         'depth': depth_path[: train_Num],\
                         'seg': seg_path[: train_Num],}
        valid_Dataset = {'input': train_path[train_Num: train_Num + test_Num], \
                         'cholec80': cholec80_path[train_Num: train_Num + test_Num],\
                         'depth': depth_path[train_Num: train_Num + test_Num],\
                         'seg': seg_path[train_Num: train_Num + test_Num]}
        test_Dataset = {'input': train_path[train_Num + test_Num:], \
                        'cholec80': cholec80_path[train_Num + test_Num:],\
                        'depth': depth_path[train_Num + test_Num:],\
                        'seg': seg_path[train_Num + test_Num:]}
    ...
    def k_folder(self,seed =1024,test_radio = 0.2, \
        ...
        for i, split_Point in enumerate(split_Points[:-1]):
            ...
            k_train_C_path = cholec80_path[0: before] + cholec80_path[after: end_point]
            k_train_D_path = depth_path[0: before] + depth_path[after: end_point]
            k_valid_I_path = train_path[before: after]
            k_valid_C_path = cholec80_path[before: after]
            k_valid_D_path = depth_path[before: after]
            train_Dataset = {'input': k_train_I_path, 'cholec80': k_train_C_path,\
                             "depth": k_train_D_path}
            valid_Dataset = {'input': k_valid_I_path, 'cholec80': k_valid_C_path,\
                             "depth": k_valid_D_path}
            ret_Dic = {}
            ret_Dic['train'] = train_Dataset
            ret_Dic['valid'] = valid_Dataset
            output_dict_pth = os.path.join(output_folder, file_Name)
        ...
        test_Dataset = {'input': train_path[end_point:], \
                        'cholec80': cholec80_path[end_point:]}
        ret_Dic['test'] = test_Dataset
        with open(output_dict_pth, 'w', encoding='utf-8') as f:
            json.dump(ret_Dic, f, indent=4)
