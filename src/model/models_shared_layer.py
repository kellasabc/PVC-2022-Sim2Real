...
class Generator(nn.Module):
   ...
   def __init__(self, in_channels, out_channels_cholec80, out_channels_depth, \
       ...
       self.decoders_sharedlayer = nn.ModuleList(self.decoders_sharedlayer)
   ...
   def forward(self, x):
       ...
       skips_cons_shared = skips_cons_cholec80[:3]
       skips_cons_individual = skips_cons_cholec80[3:]
       decoders_sharedlayer = self.decoders_sharedlayer
       decoders_cholec80 = self.decoders_cholec80[:-1]
       decoders_depth = self.decoders_depth[:-1]
       decoders_seg = self.decoders_seg[:-1]
       for decoder, skip in zip(decoders_sharedlayer, skips_cons_shared ):
           x = decoder(x)
           # print(x.shape, skip.shape)
           x = torch.cat((x, skip), axis=1)
       x_depth = torch.clone(x)
       x_seg = torch.clone(x)
       for decoder, skip in zip(decoders_cholec80, skips_cons_individual):
           x = decoder(x)
           # print(x.shape, skip.shape)
           x = torch.cat((x, skip), axis=1)
       for decoder, skip in zip(decoders_depth, skips_cons_individual):
           x_depth = decoder(x_depth)
           # print(x.shape, skip.shape)
           x_depth = torch.cat((x_depth, skip), axis=1)
       for decoder, skip in zip(decoders_seg, skips_cons_individual):
           x_seg = decoder(x_seg)
           # print(x.shape, skip.shape)
       ...
       return self.sigmoid(x), self.sigmoid(x_depth), self.sigmoid(x_seg)
