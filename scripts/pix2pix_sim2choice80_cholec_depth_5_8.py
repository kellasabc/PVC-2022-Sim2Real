...
from src.utils.utils_loss import DiceBCELoss
...
class pix2pix():
    ...
    def __init__(self, current_device, batch_size, json_path, log_file,saved_model_file,\
        ...
        self.train_dataset = [train_dataset[-i]for i in range(1,9) ]
        ...
        self.Dice_loss = DiceBCELoss(ratio=(0.8,0.2)).to(device=self.device)
    ...
    def train(self, epochs=100, start_epoch=0):
        ...
        for epoch in range(start_epoch+1, epochs + start_epoch+ 1):
            ...
            for imgs, labels in self.train_loader:
                ...
                cross_loss_seg = self.ratio_seg * \
                                 self.Dice_loss(F.softmax(gen_seg[:, :, :, :], dim=1).float(), \
                                                 F.one_hot(labels_[:, 4, :, :].to(torch.long), \
                                                           self.n_class).permute(0, 3, 1, 2).float())
    ...
    def test(self, save_path='../../result/generated test images'):
        ...
        with torch.no_grad():
            for cur_num, (ori_path, img, target, target_ori) in enumerate(self.test_dataset):
                ...
                gen_image_ = gen_image.clone()
                for channel in range(3):
                    gen_image_[:, channel, :, :] = (gen_image_[:, channel, :, :] - self.norm_param[
                        1, channel, 0]) * self.metrics / \
                                                   (self.norm_param[1, channel, 1] -
                                                    self.norm_param[1, channel, 0])
                ...
                D_fake_decision = self.D(img_, gen_image_)
                ...
                # L1 loss
                l1_loss_cholec_80 = self.ratio_cholec * self.L1_loss(gen_image_[:, :3, :, :], target_[:, :3, :, :])
                l1_loss_depth = self.ratio_depth * self.L1_loss(gen_image_[:, 3:4, :, :], target_[:, 3:4, :, :])
                cross_loss_seg = self.ratio_seg * self.Cross_loss(gen_seg[:, :, :, :], target_[:, 4, :, :]. \
                                                                  to(torch.long))
                l1_loss = l1_loss_cholec_80 + l1_loss_depth + cross_loss_seg
    ...
    def train_generate(self, save_path='../../result/generated train images'):
        """
        work on test phase of model: generate image and monitor image in test phase
        :param save_path: the path you want to save these generated images in test phase
        :return: None
        """
        print("*" * 30)
        print("train generate image phase")
        print("*" * 30 + "\n")
        self.G.eval()
        G_losses = []
        D_losses = []
        ...
        if os.path.exists(save_path):
            shutil.rmtree(save_path)
        os.mkdir(save_path)
        monitor_path = save_path.replace('generated train images', 'monitor_train')
        if os.path.exists(monitor_path):
            shutil.rmtree(monitor_path)
        os.mkdir(monitor_path)
        with torch.no_grad():
            for cur_num, (img, target) in enumerate(self.train_dataset):
                img = img.to(device=self.device)
                target = target.to(device=self.device)
                img_ = img.clone().unsqueeze(0)
                target_ = target.clone().unsqueeze(0)
                ...
                G_losses.append(l1_loss.item())
                D_losses.append(D_loss.item())
                self.writer.add_text("train generate image", ' {}, L1 loss {}, Dis loss {}'.format( \
                    cur_num, l1_loss.item(), D_loss.item()))
                print(' {}, L1 loss {}, Dis losss {}'.format(cur_num, l1_loss.item(), \
                                                             D_loss.item()))
                # Scale to 0-255
                fig, axes = plt.subplots(1, 7, figsize=(5, 5))
                imgs = [img, gen_image[-1][:3,:,:], target[:3, :, :], \
                        gen_image[-1][3:4,:,:], target[3:4, :, :], \
                        gen_image[-1][4:,:,:], target[4:, :, :], ]
                imgs_key = ['input', 'generate_cholec80', 'cholec80', \
                            "generate_depth", 'depth', \
                            "generate_seg", "seg"]
                show_imgs = []
                ...
                plt.subplots_adjust(wspace=0, hspace=0)
                plt.show()
                monitor_path_cur = os.path.join(monitor_path, str(cur_num)+".png")
                filepath_monitor, _ = os.path.split(monitor_path_cur)
                if not os.path.exists(filepath_monitor):
                    os.makedirs(filepath_monitor)
                for i in [3,4,5,6]:
                    show_imgs[i] = np.concatenate((show_imgs[i], show_imgs[i], show_imgs[i]), axis=2)

                monitor_img_1 = np.concatenate((show_imgs[0], show_imgs[2], show_imgs[4], show_imgs[6]), axis=1)
                monitor_img_2 = np.concatenate((show_imgs[0], show_imgs[1], show_imgs[3], show_imgs[5]), axis=1)
                monitor_img = np.concatenate((monitor_img_1, monitor_img_2), axis=0)
                Image.fromarray(monitor_img).save(monitor_path_cur, "PNG")
        print("the average loss of test is {}".format(torch.mean(torch.Tensor(G_losses))))
        print("*" * 30)
